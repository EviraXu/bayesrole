...
def positional_encoding(position, d_model):
    def get_angles(pos, i, d_model):
        pos = tf.cast(pos, tf.float32)  # 确保pos是float32类型
...
def create_model_transformer(sequence_length=65, d_model=64, num_heads=1, ff_dim=128, num_layers=1, dropout_rate=0.1):
    ...
    win_probs = Dense(60, activation='sigmoid')(x)
    print("win_probs shape:",win_probs.shape)
    ...
    #optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)  # 使用Adam优化器并设置初始学习率
...
def train(num_succeeds, num_fails, propose_count, model_type):
    ...
    if model_type == 'win_probs':
        checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
            'winprobs_models/{}_{}_{}.h5'.format(num_succeeds, num_fails, propose_count),
            save_best_only=True
        )
        tensorboard_callback = tf.keras.callbacks.TensorBoard(
            log_dir='winprobs_logs/{}_{}_{}'.format(
                num_succeeds,
                num_fails,
                propose_count
            )
        ...
        tensorboard_callback = tf.keras.callbacks.TensorBoard(
            log_dir='transformer_logs/{}_{}_{}'.format(
                num_succeeds,
                num_fails,
                propose_count
            )
...
def held_out_loss_data(num_succeeds, num_fails, propose_count):
    ...
    val_X, train_X = X[:4000], X[4000:]
    val_Y, train_Y = Y[:4000], Y[4000:]
    #进行模型训练和验证的循环
    for model_type in ['unconstrained', 'win_probs','transformer']:
        #for n_datapoints in [20000, 40000, 60000, 80000, 100000]:
        for n_datapoints in [4000, 8000, 12000, 16000, 20000]:
            epochs = int(3000 * 20000/n_datapoints)
            patience = int(100 * 20000/n_datapoints)
