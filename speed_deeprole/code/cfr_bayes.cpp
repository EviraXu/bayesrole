...
//计算游戏策略.累积策略权重参数 cum_strat_weight
void calculate_strategy(LookaheadNode* node, const double cum_strat_weight) {
    ...
    case PROPOSE: {
        ...
        if (cum_strat_weight != 0) {//如果累积策略权重 cum_strat_weight 不为零
...
void cfr_get_values(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        //根据当前迭代次数 iter 和 wait_iterations，计算权重 weight
        ...
        for (int i = 0; i < NUM_PLAYERS; i++) {
            ...
            // std::cout << "Player:" << i <<" isApprox:";
            // std::cout << flag << std::endl;
            double mse = (last_values[i] - root->counterfactual_values[i]).matrix().squaredNorm() / NUM_VIEWPOINTS;
            strategy_change += mse;
        }
