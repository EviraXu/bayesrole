...
class AvalonState(GameState):
    ...
    def legal_actions(self, player, hidden_state):
        ...
        #如果当前状态是选择梅林，那么合法的行动是PickMerlinAction，为游戏中的每个玩家创建一个此类行动，即任何玩家都可能被选为梅林。
        if self.status == 'merlin':
            return [PickMerlinAction(merlin=p) for p in range(self.NUM_PLAYERS)]

        #合法的行动是ProposeAction，包括所有可能的玩家组合作为任务队伍的提案
        if self.status == 'propose':
        ...
        #合法的行动有两种：投赞成票（VoteAction(up=True)）和投反对票（VoteAction(up=False)）
        if self.status == 'vote':
            return [VoteAction(up=True), VoteAction(up=False)]

        #执行任务
        #邪恶角色（EVIL_ROLES）可以选择使任务成功（fail=False）或失败（fail=True），而善良角色只能选择使任务成功。
        if self.status == 'run':
